    ...
    return {
      handleFiles: function (oEvent) {
        ...
        const processData = (csv) => {
          ...
          this.getView().setModel(
            ...
            "SupplierUsersDataToImport"
          );

          sap.ui
            .getCore()
            .byId(
              "com.sap.pgp.dev.SupplierControlApp.fragment.ImportSupplierUsersDialog--UploadSupplierUser"
            )
            .getItems()[0]
      ...
      onStartUpload: function (oEvent) {
        this.getView()
          ...
          .setProperty("/isSupplierUserImportLoading", true);
        var AdminModel = this.getOwnerComponent().getModel("AdminMo");
        const payloadData = this.getView().getModel(
          "SupplierUsersDataToImport"
        ).oData;

        AdminModel.create("/DoLoadSupplierUsers", payloadData, {
          success: (oRetrievedResult) => {
            this.getView()
              .getModel("SuppliersModel")
              .setProperty("/isSupplierUserImportLoading", false);
            MessageToast.show(oRetrievedResult.DoLoadSupplierUsers);
          },
