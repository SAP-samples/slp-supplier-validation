sap.ui.define(
  ...
  function (
    ...
    return BaseController.extend("com.sap.pgp.dev.SupplierControlApp.controller.App", {
      _bExpanded: true,
        ...
        oList.addEventDelegate(
          {
            onAfterRendering: function () {
              const crrentRoute = window.location.hash.toString();
              var oItems = this.getItems();

              for (var i = 0; i < oItems.length; i++) {
                let check = false;
                if (oItems[i].mProperties.href == crrentRoute) {
                  check = true;
                }
                if (oItems[i].mAggregations.items.length > 0) {
                  for (
                    var j = 0;
                    j < oItems[i].mAggregations.items.length;
                    j++
                  ) {
                    if (
                      oItems[i].mAggregations.items[j].mProperties.href ==
                      crrentRoute
                    ) {
                      check = true;
                    }
      ...
      onRouteChange: function (oEvent) {
        this.getModel("side").setProperty(
          "/selectedKey",
      ...
      getUserInfo: function () {
        ...
        var fnSuccess = function (data) {
          var userDetails = JSON.parse(data.d.doGetUserInformation);
          ...
          if (userDetails.roles) {
            ...
            if (Boolean(authenticatedUser)) {
              var completeNav = _this.getModel("side").oData;
              var { navigation } = completeNav;

              const homeNav = navigation.filter((data) => data.key == "Home");
              newNavigation = {
                ...newNavigation,
                navigation: [...newNavigation.navigation, ...homeNav],
              };
              ...
              if (Boolean(DataLakeEnabled)) {
                const confUploadNav = navigation.filter(
                  (data) => data.key == "ConfigurationUpload"
                );
                if (confUploadNav.length && confUploadNav[0].items.length) {
                  let filteredConfUploadNav = confUploadNav[0].items.filter(
                    (data) =>
                      data.key != "Categories2RegionMap" &&
                      data.key != "Categories2SupplierMap"
                  );
                  confUploadNav[0].items = filteredConfUploadNav;
                }

                newNavigation = {
                  ...newNavigation,
                  navigation: [...newNavigation.navigation, ...confUploadNav],
                };
              }

              if (Boolean(CategoryAdmin)) {
                const tentativeNav = navigation.filter(
                  (data) => data.key == "ConfigurationUpload"
                );

                newNavigation = {
                  ...newNavigation,
                  navigation: [...newNavigation.navigation, ...tentativeNav],
                };
              }

              if (Boolean(ReportingManager)) {
                const tentativeNav = navigation.filter(
                  (data) => data.key == "statistics"
                );
                newNavigation = {
                  ...newNavigation,
                  navigation: [...newNavigation.navigation, ...tentativeNav],
              ...
              if (Boolean(CategoryManager)) {
                const tentativeNav = navigation.filter(
                  (data) => data.key == "SupplierManagement"
                );
                newNavigation = {
                  ...newNavigation,
                  navigation: [...newNavigation.navigation, ...tentativeNav],
              ...
              if (Boolean(SuperAdmin)) {
                newNavigation = completeNav;
              }

              _this.getModel("side").setProperty("/", newNavigation);
            }
          } else {
        ...
        $.ajax({
          type: "POST",
          url: "/backend/v2/sourcing/doGetUserInformation",
          headers: {
          ...
          .then(
            function (oBundle) {
              ...
              var fnHandleUserMenuItemPress = function (oEvent) {
                debugger;
                this.getBundleText("clickHandlerMessage", [
                  oEvent.getSource().getText(),
                ]).then(function (sClickHandlerMessage) {
      ...
      _createNotification: function (sId, oBindingContext) {
        ...
        var oNotificationItem = new NotificationListItem({
          ...
          close: function (oEvent) {
            ...
            this.getBundleText("notificationMessageDeleted").then(function (
              sMessageText
            ) {
      ...
      _handleWindowResize: function (oDevice) {
        if (
          (oDevice.name === "Tablet" && this._bExpanded) ||
      ...
      fetchSiteControlParams: async function () {
        var oDataModel = this.getOwnerComponent().getModel("AdminMo");

        oDataModel.read("/SiteControlParams", {
          success: (oData) => {
            if (oData.results.length)
              this.setModel(
                new JSONModel({ ...oData.results[0], dataAvailable: true }),
                "SiteControlParams"
              );
          },
          error: function (oError) {
            console.log(oError, "oError");
            MessageToast.show("Error while fetching the SiteControlParams");
          },
        });
      },
    });
